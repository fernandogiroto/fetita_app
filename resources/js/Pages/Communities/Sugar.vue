                  ...
                  <div class="card lex-grow-1">
                    ...
                    <div class="card-footer p-0">
                      <form @submit.prevent="subscribe">
                        <button class="btn btn-primary btn-square w-100 p-3" v-if="!user.cm_sugar">
                          Me Inscrever na Comunidade
                        </button> 
                        <button class="btn btn-primary btn-square w-100 p-3" v-else>
                          Sair da Comunidade
                        </button> 
                      </form>
...
onMounted(() => {
  inscription.value = props.user.cm_sugar;
})

const form = useForm({
    subscribe: inscription.value,
});

const subscribe = () => {
  console.log(inscription.value)
  console.log(props.user.cm_sugar)
    form.put(route('comunidades.sugar.subscribre'), {
        preserveScroll: true,
        onSuccess: () => {
          inscription.value = !inscription.value
        }
    });
};
</script>